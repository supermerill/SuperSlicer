...
SCENARIO("Region expansion basics", "[RegionExpansion]") {
    static constexpr const coord_t ten = scale_i(10.);
    GIVEN("two touching squares") {
        ...
        static constexpr const float expansion = scale_d(1.);
    ...
    GIVEN("simple bridge") {
        ...
        WHEN("fully expanded") {
            static constexpr const float expansion = scale_d(10.1);
            std::vector<Polygons> expanded = Algorithm::expand_expolygons({ ExPolygon{square2} }, { ExPolygon{square1}, ExPolygon{square3} },
                expansion,
                scale_d(2.3), // expansion step
                5);                 // max num steps
            THEN("Two anchors are produced") {
                REQUIRE(expanded.size() == 1);
                REQUIRE(expanded.front().size() == 2);
        ...
        WHEN("expanded") {
            ...
            SVG::export_expolygons(DEBUG_TEMP_DIR "two_bridges-out.svg",
                { { { { ExPolygon{left_support}, ExPolygon{right_support} } }, { "supports", "orange", 0.5f } },
                  ...
                  { { union_ex(union_(expanded.front(), expanded.back())) },   { "expanded", "red",    "black", "", scale_i(0.1f), 0.5f } } });
#endif
        ...
        WHEN("expanded") {
            ...
            SVG::export_expolygons(DEBUG_TEMP_DIR "rectangle_with_rhombic_cut-out.svg",
                { { { { ExPolygon{square_with_rhombic_cutout} } }, { "square_with_rhombic_cutout", "orange", 0.5f } },
                  ...
                  { { union_ex(expanded.front()) },                { "bridges",                    "red",    "black", "", scale_i(0.1f), 0.5f } } });
#endif
            THEN("Single anchor is produced") {
                REQUIRE(expanded.size() == 1);
            }
            THEN("The area of anchor is correct") {
                ...
                REQUIRE(is_approx(area_expected, area_calculated, sqr(scale_d(0.2))));
            }
        }

        WHEN("extra expanded") {
            static constexpr const float expansion = scale_d(2.5);
            std::vector<Polygons> expanded = Algorithm::expand_expolygons({ ExPolygon{rhombic} }, { ExPolygon{square_with_rhombic_cutout} },
                expansion,
                scale_d(0.25), // expansion step
                11);                 // max num steps
#if 0
            ...
            THEN("The area of anchor is correct") {
                double area_calculated = area(expanded.front());
                double area_expected = 2. * diag * expansion + M_PI * sqr(expansion) * 0.75;
                REQUIRE(is_approx(area_expected, area_calculated, sqr(scale_d(0.3))));
            }
        }
    }
    ...
    GIVEN("square with two holes") {
        ...
        WHEN("expanded") {
            ...
            SVG::export_expolygons(DEBUG_TEMP_DIR "square_with_two_holes-out.svg",
                { { { { ExPolygon{anchor} } }, { "anchor", "orange", 0.5f } },
                  ...
                  { { union_ex(expanded.front()) }, { "expanded", "red", "black", "", scale_i(0.1f), 0.5f } } });
#endif
            ...
            THEN("The area of anchor is correct") {
                ...
                REQUIRE(is_approx(area_expected, area_calculated, sqr(scale_d(0.45))));
            }
        }
        WHEN("expanded even more") {
            static constexpr const float expansion = scale_d(25.);
            std::vector<Polygons> expanded = Algorithm::expand_expolygons({ ExPolygon{anchor} }, { boundary },
                expansion,
                scale_d(2.), // expansion step
                15);               // max num steps
#if 0
        ...
        WHEN("expanded yet even more") {
            static constexpr const float expansion = scale_d(28.);
            std::vector<Polygons> expanded = Algorithm::expand_expolygons({ ExPolygon{anchor} }, { boundary },
                expansion,
                scale_d(2.), // expansion step
                20);               // max num steps
        ...
        WHEN("expanded fully") {
            static constexpr const float expansion = scale_d(35.);
            std::vector<Polygons> expanded = Algorithm::expand_expolygons({ ExPolygon{anchor} }, { boundary },
                expansion,
                scale_d(2.), // expansion step
                25);               // max num steps
    ...
    GIVEN("square with hole, hole edge anchored") {
        ...
        WHEN("expanded") {
            ...
            THEN("The area of anchor is correct") {
                ...
                REQUIRE(is_approx(area_expected, area_calculated, sqr(scale_d(0.6))));
            }
        }
    }
}
