...
bool OptionsSearcher::search(const std::string& search,  bool force/* = false*/)
{
    ...
    for (size_t i=0; i < options.size(); i++)
    {
        ...
        if (full_list) {
            ...
            found.emplace_back(FoundOption{ label, label, boost::nowide::narrow(get_tooltip(opt, true).wc_str()), i, 0 });
            continue;
        }
...
#endif
	        found.emplace_back(FoundOption{ label_plain, label_u8, boost::nowide::narrow(get_tooltip(opt, true).wc_str()), i, score });
        }
    }
