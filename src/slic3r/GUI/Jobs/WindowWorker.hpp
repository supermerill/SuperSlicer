...
class WindowWorker: public Worker {
    ...
    public:
        void process(Ctl &c) override
        {
            ...
            // messages in window's wxIdle handler. Otherwise it might happen
        ...
        void finalize(bool canceled, std::exception_ptr &eptr) override
        {
            ...
            if (eptr) try {
                    ...
                    show_error(m_window, _L("An unexpected error occured: ") + e.what());
                    eptr = nullptr;
                }
        }

        WindowJob(wxWindow *w, std::unique_ptr<Job> j)
            : m_job{std::move(j)}, m_window{w}
        {}
...
public:
    ...
    WindowWorker(wxWindow *window, WorkerArgs &&...args)
        : m_w{std::forward<WorkerArgs>(args)...}, m_window{window}
    {
        // Ensure that messages from the worker thread to the UI thread are
        // processed continuously.
        window->Bind(wxEVT_IDLE, [this](wxIdleEvent &) {
            process_events();
        });
    }

    // Always package the job argument into a WindowJob
    bool push(std::unique_ptr<Job> job) override
    {
        return m_w.push(std::make_unique<WindowJob>(m_window, std::move(job)));
    }
