...
void resolve_path_from_var(const std::string& var, std::vector<std::string>& paths)
{
    ...
    if (! wxGetEnv(boost::nowide::widen(var), &wxdirs) || wxdirs.empty() )
        return;
    std::string dirs = boost::nowide::narrow(wxdirs.wc_str());
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        ...
        if (i == target_candidates.size() - 1) {
            // create $HOME/.local/share
              create_path(boost::nowide::narrow(wxFileName::GetHomeDir().wc_str()), ".local/share/icons" + icon_theme_dirs);
              // copy icon
    ...
    if (!candidate_found) {
        // create $HOME/.local/share
        create_path(boost::nowide::narrow(wxFileName::GetHomeDir().wc_str()), ".local/share/applications");
        // create desktop file
