...
EPropertySpeedFieldEditor remembers which EPropertySpeed payloads it creates. This
...
{
    return leaf_disposition(entity, state, EPropertySpeedField::Speed);
}
                                 ...
                                 EPropertySpeedField field)
{
    ...
    if (field == EPropertySpeedField::FanSpeed) {
...
EPropertySpeedFieldEditor::EPropertySpeedFieldEditor(EPropertySpeedField field) : m_field(field) {}

float EPropertySpeedFieldEditor::state_value(const EffectiveTreeState &state) const
{
    switch (m_field) {
    case EPropertySpeedField::Speed: return state.speed;
    case EPropertySpeedField::Acceleration: return state.acceleration;
    case EPropertySpeedField::FanSpeed: return state.fan_speed;
    }
    return -1.f;
}

bool EPropertySpeedFieldEditor::value_is_set(float value) const
{
    return m_field == EPropertySpeedField::FanSpeed ? value >= 0.f : value > 0.f;
}

float EPropertySpeedFieldEditor::direct_value(const EPropertySpeed *process) const
{
    if (process == nullptr)
        return -1.f;
    switch (m_field) {
    case EPropertySpeedField::Speed: return process->speed_mm_per_s;
    case EPropertySpeedField::Acceleration: return process->accel_mm_per_s2;
    case EPropertySpeedField::FanSpeed: return process->fan_speed_percent;
    }
    return -1.f;
}

float &EPropertySpeedFieldEditor::field(EPropertySpeed &process) const
{
    switch (m_field) {
    case EPropertySpeedField::Speed: return process.speed_mm_per_s;
    case EPropertySpeedField::Acceleration: return process.accel_mm_per_s2;
...
EPropertySpeed &EPropertySpeedFieldEditor::ensure_property(MutableExtrusionEntity entity)
{
...
void EPropertySpeedFieldEditor::set_value(MutableExtrusionEntity entity, float value)
{
...
EPropertySpeedFieldEditor::FieldSummary EPropertySpeedFieldEditor::merge_fields(
...
void EPropertySpeedFieldEditor::clear_redundant_field(MutableExtrusionEntity entity,
                                                      float inherited_value)
{
    ...
    if (m_field == EPropertySpeedField::FanSpeed &&
...
EPropertySpeedFieldEditor::FieldSummary EPropertySpeedFieldEditor::hoist_tree(
    ...
    const EffectiveTreeState &parent_state)
{
    ...
    if (summary.has_value && !summary.blocked && (!value_is_set(direct) || direct == summary.value)) {
        if (!value_is_set(direct) && state_value(state) != summary.value) {
            ...
            m_modified_entities.insert(entity.mutable_handle());
            if (m_field == EPropertySpeedField::Speed)
                state.speed = summary.value;
            else if (m_field == EPropertySpeedField::Acceleration)
...
void EPropertySpeedFieldEditor::hoist(MutableExtrusionEntity root)
{
